...
/**
 * Background image generation logic, shared by /api/generate-all-images,
 * /api/regenerate-images, and /api/generate-project (fixed-carousel auto-trigger).
 *
 * Lives outside any route.ts file because Next.js's app-router type checking only
 * allows route.ts files to export recognized route handlers (GET/POST/etc.) — any
 * other export (like this function) fails the build's type check.
 */
